...
function GetBottle(image, rect)
{
	...
	for (let i = 0; i < 4; i++)
	{
		...
		let color = GetColor(image, x, y, g_mitidalu_color);
		bottle.setColor(i, color);
		//Rect rect(x - 1, y - 1, 3, 3);
		//rectangle(image, rect, cv::Scalar(0, 0, 255), 2, LINE_8);
	}
...
solver.init = function (src) {
	let dst = new cv.Mat();
	let gray = new cv.Mat();
	cv.cvtColor(src, gray, cv.COLOR_RGBA2GRAY, 0);
	let low = new cv.Mat(gray.rows, gray.cols, gray.type(), [75, 0, 0, 0]);
	...
	var index = 0;
	for (let key in g_mitidalu_color)
	{
		problem.color[index] = g_mitidalu_color[key][3];	// 最后一个是给网页显示用的
		index++;
	}
	...
	for (let key in bottleLines)
	{
		...
		{
			var bl = GetBottle(src, bottles[bottlekey]);
